...
【高エンゲージメントパターン（分析結果より）】
以下の書き出しパターンは実績で20%超のエンゲージメント率を記録している。積極的に使うこと:
- 「〜、しんどかった。」で始まる苦労・挫折談
- 「〜のトリガー、また壊れた。」など具体的な失敗報告
- 「〜始めて気づいたこと。」で始まる学びの共有
- 「最初の○ヶ月は〜」で始める期間を示した体験談
- 「〜が怖くて使えなかった。でも実際は…」という克服の物語
これらのパターンは説教ではなく"現在進行形の同志"として書くこと。
"""
...
- AxeonPost（AI X自動投稿SaaS）を開発・Vercelにデプロイ済み
- note記事を13本公開（無料記事〜¥1,980の技術解説まで）
- アフィリエイト自動投稿も並行稼働（転職・クレカ・光回線系の高単価商材）
- X API・Claude API・Supabase・Next.jsを組み合わせた開発経験あり
- X Post Scheduler（Electron製デスクトップアプリ）を開発・ビルド完了
- 3ヶ月で約289投稿、22,284インプレッション、403いいね獲得

【リアルな失敗・しんどかったこと】
- 最初の1ヶ月はフォロワーが全く増えず「自分には無理かも」と何度も思った
- Supabaseのトリガーが何度もエラーを吐いて投稿スケジュールが崩壊した
- VPSの設定でハマって3時間溶かしたことがある
- X APIのクレジット不足で投稿が止まり、気づくまで2日かかった
- 完璧なシステムを作ってから発信しようと思って、1ヶ月何も発信できなかった
- Claude APIの料金が怖くて最初は試せなかった（実際は1投稿1円以下だった）
"""

# スロット名: (開始時刻, 投稿タイプ, 方向性)
TIME_SLOTS = {
    "morning": (7,  "教育系",  "AIや自動化について『知らなかった・得した』と感じるノウハウ。実際に試してわかったこと・具体的な数値を盛り込む。箇条書き推奨。失敗してから学んだ形式（『〜してみたら〇〇だった』『最初は怖かったが実際は〜』）だとなお良い。"),
    "noon":    (12, "共感系",  "副業・AI活用で感じるリアルな悩みや挫折・気づき。『しんどかった』『自分だけじゃなかった』と思わせる内容。感情を正直に書く。特に『最初の〇ヶ月は〜』『〜できなかった。でも続けたら〜』という克服パターンはエンゲージメントが高い。"),
    "evening": (21, "実録系",  "今日やったこと・わかったこと・失敗したことを日記感覚で正直に。『〜のトリガーまた壊れた』『〜でハマって〇時間溶かした』など具体的な失敗報告が特に刺さる。数字や固有名詞（Supabase・Claude API等）を入れるとリアリティが増す。"),
}

# スレッドを投稿する曜日（0=月, 1=火, 2=水, 3=木, 4=金, 5=土, 6=日）
# 分析結果: 水曜108.9imp・火曜93.8imp が最高。金曜72.7impは平均以下なので水曜に変更。
THREAD_DAYS = [1, 2]  # 火曜・水曜

# スレッドのトピック候補（順番に使い回す）
# 分析結果: 体験談・失敗談系が高エンゲージメント。トピックも個人体験ベースを優先。
THREAD_TOPICS = [
    "AI副業を始めて最初の3ヶ月でしんどかったこと・乗り越えた方法",
    "Claude APIとPythonでX自動投稿を作った全手順【失敗含め全部公開】",
    "会社員がAI副業を始めて最初の1ヶ月でやったこと・失敗したこと",
    "Supabaseのトリガーで詰まりまくった話【原因と対策を全部書く】",
    "VPS上でcronを動かしてPCなしで自動投稿する仕組みを解説",
    "noteで13本書いてわかったこと【売れた記事と売れなかった記事の違い】",
    "Claude APIが怖くて使えなかった自分が、今は月〇〇円で運用している話",
    "アフィリエイト自動投稿システムを作った話【設計から実装まで】",
    "AIで副業収益化するまでのリアルなロードマップ【現在地も公開】",
    "X Post SchedulerというElectronアプリを作ってGumroadで売るまでの記録",
]
